...
function reacterminator ({type, content}, options) {
  ...
  function createProcessFunction (processName, type) {
    return (initialValue) => {
      const { component, components } = options.pipThroughPlugins(processName, initialValue)
      return type === 'each' ? component : components;
    }
  }
  ...
  const components = pips.reduce((previousComponents, {fn, type}) => {
    ...
    const processFunction = _.isFunction(fn) ? fn : createProcessFunction(fn, type)
