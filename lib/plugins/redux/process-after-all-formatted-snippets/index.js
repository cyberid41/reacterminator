...
module.exports = function processAfterAllFormattedSnippets({ components, options }) {
  ...
  // custom folders
  shell.mkdir(path.resolve(outputPath, 'custom/action-type-constants'));
  shell.mkdir(path.resolve(outputPath, 'custom/action-creators'));
  shell.mkdir(path.resolve(outputPath, 'custom/reducers'));
