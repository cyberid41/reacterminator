...
function addStateAndAction(
  { nodeState, nodeAction, nodeErrorState, nodeErrorAction }, state, action
) {
  // NOTE: nodeState and nodeAction can be array or value
  if (nodeState) { state.push(nodeState); }
  if (nodeErrorState) { state.push(nodeErrorState); }
  if (nodeAction) { action.push(nodeAction); }
  if (nodeErrorAction) { action.push(nodeErrorAction); }
}
